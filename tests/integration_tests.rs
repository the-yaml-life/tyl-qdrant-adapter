...
async fn test_vector_operations_integration() {
    ...
    let search_results = adapter.search_similar("vectors_test", query_vector, search_params).await;
...
async fn test_error_handling_integration() {
    ...
    let result = adapter.get_vector("non_existent_collection", "non_existent_id").await;
    assert!(result.is_err());
    
    // Test searching in non-existent collection
    let search_params = SearchParams::with_limit(10);
    let result = adapter.search_similar("non_existent_collection", vec![0.1; 128], search_params).await;
    assert!(result.is_err());
    
    // Test deleting from non-existent collection
    let result = adapter.delete_vector("non_existent_collection", "some_id").await;
    assert!(result.is_err());
