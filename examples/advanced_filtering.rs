...
use tyl_qdrant_adapter::{
    CollectionConfig, DistanceMetric, MockQdrantAdapter, SearchParams, Vector,
    VectorCollectionManager, VectorStore,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match adapter
        .search_similar("products", query_vector, search_params)
        .await
    {
        Ok(results) => {
            println!(
                "📋 Found {} products in price range $200-$1000:",
                results.len()
            );
    ...
    let search_params = SearchParams::with_limit(5).with_filter("category", in_filter);
        ...
        .await
    {
        Ok(results) => {
            println!(
                "📋 Found {} products in categories [electronics, books]:",
                results.len()
            );
            for result in results {
                println!("   • {} (score: {:.3})", result.vector.id, result.score);
    ...
    let search_params = SearchParams::with_limit(5).with_filter("premium", exists_filter);
        ...
        .await
    {
        Ok(results) => {
            println!(
                "📋 Found {} premium electronics under $1200:",
                results.len()
            );
            for result in results {
                println!("   • {} (score: {:.3})", result.vector.id, result.score);
    ...
    let search_params =
        SearchParams::with_limit(5).with_filter("category", serde_json::json!("books"));
        ...
        .await
    {
        Ok(results) => {
            println!(
                "📋 Found {} books using traditional filtering:",
                results.len()
            );
            for result in results {
                println!("   • {} (score: {:.3})", result.vector.id, result.score);
