...
use tyl_qdrant_adapter::{VectorDatabase, VectorStore, VectorCollectionManager, VectorStoreHealth};
...
async fn demonstrate_advanced_filtering(
    ...
    println!("   Found {} high-priority published documents", results.len());
    ...
    println!("   Found {} published and featured documents", results.len());
    for result in &results {
        let title = result.vector.metadata.get("title")
            .and_then(|v| v.as_str()).unwrap_or("N/A");
        let priority = result.vector.metadata.get("priority")
            .and_then(|v| v.as_i64()).unwrap_or(0);
        println!("   - {} (priority: {}, score: {:.3})", title, priority, result.score);
    }
    
    // Test 5: Range filtering demonstration (using public methods)
    println!("\n📊 Test 5: Range filtering capabilities");
    if let Some(filter) = QdrantAdapter::build_range_filter("created_timestamp", Some(1640995200.0), Some(1672531200.0)) {
        println!("   ✅ Range filter created for timestamp range");
        println!("   📝 This filter would find documents created between Jan 1, 2022 and Jan 1, 2023");
    }
...
async fn demonstrate_schema_migration(
    ...
    let migration_v1 = MigrationBuilder::new(
        Version::new(1, 0, 0),
    ...
    .create_collection(
        CollectionConfig::new("users", 256, DistanceMetric::Euclidean)?
    )
    ...
    println!("   🔍 Pact validation: {}", if result.pact_validation_passed { "✅ PASSED" } else { "❌ FAILED" });
    ...
    .create_collection(
        CollectionConfig::new("analytics", 512, DistanceMetric::DotProduct)?
    )
    ...
    let migration_v2 = MigrationBuilder::new(
        Version::new(2, 0, 0),
        "Major schema update".to_string()
    )
    ...
    .create_collection(
        CollectionConfig::new("images", 2048, DistanceMetric::Cosine)?
    )
    .create_collection(
        CollectionConfig::new("audio", 1024, DistanceMetric::Manhattan)?
    )
    .build();
    ...
    for (i, migration) in history.iter().enumerate() {
        let breaking_indicator = if migration.metadata.breaking_change { "⚠️ " } else { "✅ " };
        println!("   {}{}. {} - {} (by {})",
            breaking_indicator,
            i + 1,
            migration.version,
        ...
        println!("      Collections: {} changes", migration.collection_changes.len());
...
fn create_rich_vector(id: &str, title: &str, status: &str, priority: i32, featured: bool) -> Vector {
    ...
    metadata.insert("created_timestamp".to_string(), serde_json::json!(1672531200.0)); // Jan 1, 2023
